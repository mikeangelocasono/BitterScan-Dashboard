...
/**
 * Admin-only API endpoint to reject a user.
...
export async function POST(request: NextRequest) {
  ...
  if (!authResult.authenticated) {
    console.warn('[/api/users/reject] Unauthorized access attempt');
    return createUnauthorizedResponse(authResult.error || 'Authentication required');
  }
  
  if (!authResult.isAdmin) {
    console.warn('[/api/users/reject] Non-admin user attempted access:', authResult.email);
    return createForbiddenResponse('Admin access required');
  }
  
  console.log('[/api/users/reject] Admin action by:', authResult.email);
  ...
  try {
    ...
    if (error) {
      console.error("[/api/users/reject] Supabase update error:", {
    ...
    if (!data) {
      return NextResponse.json(
        { error: "User not found or already rejected" },
        { status: 404 }
      );
    }
    ...
    console.error("[/api/users/reject] Unexpected error:", {
      message,
      error: err,
